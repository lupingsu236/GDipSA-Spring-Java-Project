...
class ProductUserTransactionTransactionDetailMappingTest 
{
	...
	void creationTest() 
	{
		...
		User user = new User("Liau Han Yang, Jonathan", "liauhyj", "password", "liauhyj@gmail.com", RoleType.MECHANIC, ActiveType.ACTIVE, "active", "mechanic");
		userRepo.save(user);
...
//		 update the quantity of the product in the db
		...
		User lup = new User("Su Luping", "sulp", "password", "lupingsu.lp@gmail.com", RoleType.ADMIN, ActiveType.ACTIVE, "active", "admin");
		userRepo.save(lup);
		
		User zl = new User("Xu Zhenli", "xuzl", "password", "e0427350@u.nus.edu", RoleType.ADMIN, ActiveType.ACTIVE, "active", "admin");
		userRepo.save(zl);
		
		User jus = new User("Lim Yu-De Justin", "limydj", "password", "just.incandescent@gmail.com", RoleType.ADMIN, ActiveType.ACTIVE, "active", "admin");
		userRepo.save(jus);
	}
	/**
